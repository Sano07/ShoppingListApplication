...
@HiltViewModel
class ShopMainScreenViewModel @Inject constructor(
    ...
    fun onEvent(event: ShopMainScreenEvent) {
        when(event) {
            is ShopMainScreenEvent.OnItemSave -> {
            ...
            is ShopMainScreenEvent.OnShowEditDialog -> {
    ...
    override fun onDialogEvent(event: DialogEvent) {
        when(event) {
            ...
            is DialogEvent.onConfirm -> {
                onEvent(ShopMainScreenEvent.OnItemSave)
                openDialog.value = false
                editableText.value = ""
            }
